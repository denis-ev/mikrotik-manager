...
router.post('/', requireWrite, async (req: Request, res: Response) => {
  ...
  const api_port: number = preset?.api_port ?? req.body.api_port ?? 8728;
  ...
  const ssh_port: number = preset?.ssh_port ?? req.body.ssh_port ?? 22;
...
router.put('/:id', requireWrite, async (req: Request, res: Response) => {
  const { name, device_type, notes, credential_preset_id } = req.body as {
    name?: string; device_type?: string; notes?: string;
